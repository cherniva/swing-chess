...
import static Game.LocalGame.started;
import Pieces.*;
import Utils.*;
 ...
 * @author chern
 */
public class Join extends LocalGame implements Runnable {
    ...
    public Join() {
        ...
        this.setTitle("Swing Chess: online join");
    ...
    /** {@link Host#addLog(java.lang.String) }. */
    @Override
    public void addLog(String msg) {
        if(started && !msg.contains("start")) log.append(msg);
        //if(!board.replaceMode() && !msg.equals("start\n")) log.append(msg);
    ...
    public void optionsWindow() {
        ...
        holdColor = new JButton(new ImageIcon(Textures.holdb));
    ...
    public void run() {
        //while(true) {
            try {
                client = new Socket("localhost", 5300);
                ...
                if(firstConnection) {
                    ...
                    LoadGame lg = new LoadGame(sc, thisGame, board, stopwatch);
    ...
    /** {@link Host#readData(java.lang.String)   }. */
